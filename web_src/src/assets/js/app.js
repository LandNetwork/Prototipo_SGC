...
document.addEventListener('DOMContentLoaded', function () {
	...
	document.body.addEventListener('click', function (e) {
		...
		if (target && target.hostname === window.location.hostname && target.getAttribute('href') !== '#' && !target.getAttribute('href').endsWith('login.html')) {
			if (token) {
				e.preventDefault();

				var originalUrl = target.getAttribute('href');
				var parametro = 'token=' + encodeURIComponent(token);
				var nuevaUrl = originalUrl.includes('?') ? originalUrl + '&' + parametro : originalUrl + '?' + parametro;
